(function () {
    ...
    let MenuTpl =
        ...
        '<button type="button" name="submit">Ja</button>' +
        '<button type="button" name="cancel">Nej</button>' +
        '</div>' +
        '</div>'
    ;
        ...
        for (let namespace in ESX_MENU.opened) {
            for (let name in ESX_MENU.opened[namespace]) {
                ...
                $(menu).find('button[name="submit"]').click(function () {
                	...
                	var strlength = value ? value.length : 0;
                	if(value && (menuData.minlength == null || menuData.minlength <= strlength) && (menuData.maxlength == null || menuData.maxlength >= strlength)) {
                        ESX_MENU.submit(this.namespace, this.name, this.data);
                    }else{
                		if (menuData.minlength != null && menuData.minlength > strlength) {
                            $(menu).find('[name="value"]').attr("placeholder", "Du mangler "+(menuData.minlength-strlength)+" bogstaver!");
						}else if (menuData.maxlength != null && menuData.maxlength < value.length) {
                            $(menu).find('[name="value"]').attr("placeholder", "Du har "+(strlength-menuData.maxlength)+" bogstaver for meget!");
                        }
					}
                ...
                if(menuData.placeholder != null) {
                    $(menu).find('[name="value"]').attr("placeholder", menuData.placeholder);
                }else{
                    $(menu).find('[name="value"]').attr("placeholder", "");
                }
                if(menuData.maxlength != null) {
                    $(menu).find('[name="value"]').attr("maxlength", menuData.maxlength);
                }else{
                    $(menu).find('[name="value"]').attr("maxlength", null);
                }
