...
ESX.RegisterServerCallback('esx_advancedgarage:getOwnedCars', function(source, cb)
	...
	if Config.Main.ShowVehLoc then
		MySQL.Async.fetchAll('SELECT * FROM owned_vehicles WHERE owner = @owner AND Type = @Type AND job = @job AND owned_vehicles.plate NOT IN (SELECT plate from h_impounded_vehicles); ', { -- job = NULL
			['@owner'] = xPlayer.identifier,
			['@Type'] = 'car',
			['@job'] = 'civ'
		}, function(data)
